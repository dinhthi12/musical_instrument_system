<section class="box-home all mt10">
    ...
    <ul class="listproduct02 listproduct listproduct-col3">
        @foreach ($outstandingProduct as $outstandingProduct)
            @if (is_array($outstandingProduct))
                <li>
                    <div class="bglipro">
                        <figure class="reponsive-img">
                            <a href="#" title="#">
                                <img src="{{ asset('/assets/products/' . $outstandingProduct['product_image']) }}"
                                    title="#" alt="#" />
                            </a>
                        </figure>
                        <div class="c-product-item_info">
                            <div class="label-wrap">
                                @if ($outstandingProduct['product_status'] == 0)
                                    <span class="lb-hot">HOT</span>
                                @endif
                                @if ($outstandingProduct['product_discount'] > 0)
                                    <span class="lb-sale">{{ $outstandingProduct['product_discount'] }} %</span>
                                @endif
                            </div>
                            <h4>
                                <a href="#"
                                    title="{{ $outstandingProduct['product_name'] }}">{{ $outstandingProduct['product_name'] }}</a>
                            </h4>
                            <div class="price-box">
                                <span
                                    class="c-price fl">{{ number_format($outstandingProduct['product_price'], 0, ',', '.') }}
                                    VND</span>
                                <div class="clear"></div>
                            </div>
                        </div>
                        <figure class="bginfo">
                            <h4>{{ $outstandingProduct['product_name'] }}</h4>

                            <span class="c-price">{{ number_format($outstandingProduct['product_price'], 0, ',', '.') }}
                                VND</span>
